...
psts = ('idle', 'running', 'syscall', 'gcstop', 'dead')
...
def reprM(ptr, short=False):
	if long(ptr) == 0:
		return "M:null"
	obj = ptr.dereference()
	if short:
		return "M:%d" % long(obj["id"])
	attrs = {
		"id": obj['id'],
		"procid": obj['procid'],
		"p": reprP(obj["p"], short=True),
		"curg": reprG(obj['curg'])
	}
	return "M:%(id)s procid=%(procid)s curg=<%(curg)s> p=<%(p)s>" % attrs


def reprP(ptr, short=False):
	if long(ptr) == 0:
		return "P:null"
	obj = ptr.dereference()
	if short:
		return "P:%d" % long(obj["id"])
	runq = obj["runq"]
	runqsize = long(obj["runqsize"])
	runqhead = long(obj["runqhead"])
	runqtail = long(obj["runqtail"])
	str_runq = []
	for num in xrange(runqhead, runqtail):
		str_runq.append(reprG(runq[num]))
	if runqsize > (runqtail - runqhead):
		str_runq.append("...")
	attrs = {
		"id": long(obj["id"]),
		"status": psts[int(obj["status"])],
		"schedtick": long(obj["schedtick"]),
		"syscalltick": long(obj["syscalltick"]),
		"gfreecnt": long(obj["gfreecnt"]),
		"runqsize": long(obj["runqsize"]),
		"runq": "\n    ".join(str_runq)
	}
	return (
		"P:%(id)s %(status)s schedtick=%(schedtick)s syscalltick=%(syscalltick)s" +
		" gfreecnt=%(gfreecnt)s runq[%(runqsize)s]\n    %(runq)s") % attrs


class GoThreadsCmd(gdb.Command):
	"List all OS threads."

	def __init__(self):
		gdb.Command.__init__(self, "info gothreads", gdb.COMMAND_STACK, gdb.COMPLETE_NONE)

	def invoke(self, _argv, _from_tty):
		allm = gdb.parse_and_eval("'runtime.allm'")
		for ptr in linked_list(allm, 'alllink'):
			print(reprM(ptr))


class GoProcessorsCmd(gdb.Command):
	""

	def __init__(self):
		gdb.Command.__init__(self, "info goproc", gdb.COMMAND_STACK, gdb.COMPLETE_NONE)

	def invoke(self, argv, from_tty):
		maxprocs = int(gdb.parse_and_eval("'runtime.gomaxprocs'"))
		#print "runtime.gomaxprocs: %d" % maxprocs
		allp = gdb.parse_and_eval("'runtime.allp'")
		for num in xrange(maxprocs):
			print(reprP(allp[num]))
